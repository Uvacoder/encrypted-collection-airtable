	...
	export default {
			...
			meta: [
				{
					...
					template: null,
					content:
						"EncryptedList - Curated Collection of Encrypted Tools.",
				},
		...
		methods: {
			...
			filterWithCategory: function(cat, list) {
				...
				return cat.length > 0
					? list.filter((product) => product.categories.indexOf(cat) !== -1) : [...list];
			},
			...
			isThatAll: function() {
				if (this.searchQuery.trim().length > 0) {
					...
					let filtered = this.searchWithHighlight().filter(
						...
						allResults = this.searchWithHighlight(
							this.allProducts
		...
		created() {
			...
			if (
				isDefined(this.$route.query.c) ||
				isDefined(this.$route.query.t)
			) {
